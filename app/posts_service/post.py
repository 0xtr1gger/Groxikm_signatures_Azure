    ...
    def from_web_dto(dto: dict):
        if dto.keys().__contains__("id") == True:
            if (dto.get("id") != None) or (dto.get("id") != ""):
                return Post(dto.get("id"), dto.get("login"), dto.get("email"), dto.get("signature_string"), datetime.datetime.strptime(dto.get("date"), DATE_FORMAT))
        
        return Post(uuid.uuid1(), dto.get("login"), dto.get("email"), dto.get("signature_string"), datetime.datetime.now())
...
def from_db_dto(dto: dict) -> Post:
    return Post(dto.get("_id"), dto.get("login"), dto.get("email"), dto.get("signature_string"), datetime.datetime.strptime(dto.get("date"), DATE_FORMAT))
